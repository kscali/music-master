...
function (_React$Component) {
  ...
  function Tracks() {
    ...
    _defineProperty(_assertThisInitialized(_this), "playAudio", function (prevUrl) {
      return function () {
        var audio = new Audio(prevUrl);
        ...
        if (!_this.state.playing) {
          ...
          _this.setState({
            playing: true,
            audio: audio,
            playingPreviewUrl: prevUrl
          });
        } else {
          _this.state.audio.pause();

          if (_this.state.playingPreviewUrl === prevUrl) {
            _this.setState({
              playing: false
            });
          } else {
...
function (_React$Component) {
  ...
  function App() {
    ...
    _defineProperty(_assertThisInitialized(_this), "updateQuery", function (e) {
      _this.setState({
    ...
    _defineProperty(_assertThisInitialized(_this), "searchArtist", function () {
      var artist = _this.state.artistQuery;
      fetch("".concat(API_ADDRESS, "/artist/").concat(artist)).then(function (res) {
        return res.json();
      }).then(function (json) {
        if (json.artists.total > 0) {
          ...
          fetch("".concat(API_ADDRESS, "/artist/").concat(_artist.id, "/top-tracks")).then(function (res) {
            return res.json();
          }).then(function (json) {
